    ...
    for k, v in passport.items():
        ...
        if k == 'hgt':
            if v[0:-2].isdigit():
                ...
                break
        if k == 'hcl':
            rule = re.compile(r'^#[0-9a-f]{6}$')
            if rule.search(v) is None:
                isvalid = 0
                break
        if k == 'ecl':
            rule = re.compile(r'^(amb|blu|brn|gry|grn|hzl|oth)$')
            if rule.search(v) is None:
                isvalid = 0
                break
        if k == 'pid':
            rule = re.compile(r'^[0-9]{9}$')
            if rule.search(v) is None:
                isvalid = 0
                break
    if isvalid == 1:
...
def solution(puzzle_input):
    ...
    for passport in puzzle_input:
        d = dict(x.split(":") for x in passport.split())

        if all(mandatory_field in d for mandatory_field in passport_fields):
            if not PART_TWO:
                valid_passports += 1
            else:
                valid_passports += validate_fields(d)
...
if __name__ == '__main__':
    ...
    puzzle_solution = solution(puzzle_input.split("\n\n"))
